...
import type { QuestaoCaboGuerraPersonalizada } from "@/lib/caboGuerraPersonalizado";
  ...
  return (
    <div className="flex w-full max-w-[240px] flex-col gap-1.5">
      {opcoes.map((opcao, indice) => {
        const éCorreta = revelar && indice === indiceCorreto;
        const éErradaSelecionada = selecionado === indice && status === "errado";
        return (
          <button
            key={indice}
            onClick={() => onEscolher(indice)}
            className={`rounded-lg px-3 py-2.5 text-left text-sm font-bold text-white transition active:scale-95 ${
              éCorreta
                ? "bg-[#00c264]"
                : éErradaSelecionada
...
export function CaboDeGuerraPersonalizadoCliente({
  perguntas,
  turmas,
  titulo,
}: {
  perguntas: QuestaoCaboGuerraPersonalizada[];
  turmas: Turma[];
  titulo: string;
}) {
  const som = usarSom();
  ...
  const [selecao1, setSelecao1] = useState<number | null>(null);
  const [selecao2, setSelecao2] = useState<number | null>(null);
  const [status1, setStatus1] = useState<"correto" | "errado" | null>(null);
  const [status2, setStatus2] = useState<"correto" | "errado" | null>(null);
  ...
  const perguntaAtual = perguntas[rodada - 1];
  ...
  function escolher(equipe: 1 | 2, indice: number) {
    if (!ativoRef.current || !perguntaAtual) return;
    som.tic();
    const setSelecao = equipe === 1 ? setSelecao1 : setSelecao2;
    const setStatus = equipe === 1 ? setStatus1 : setStatus2;
    setSelecao(indice);
  ...
  function proximaRodada() {
    ...
    setRodada((rodadaAnterior) => {
      if (rodadaAnterior >= totalRodadas) {
        finalizarJogo();
  ...
  if (perguntas.length === 0) {
    return (
      <main className="flex min-h-screen flex-col items-center justify-center gap-3 bg-neutral-50 px-6 text-center">
        <p className="text-lg font-bold text-neutral-900">Esta atividade não tem perguntas válidas.</p>
    ...
    return (
      <main className="min-h-screen bg-neutral-50 px-6 py-10" onPointerDown={som.destravar}>
        <div className="mx-auto max-w-3xl">
          ...
          <h1 className="mt-4 text-2xl font-bold text-neutral-900">🪢 Cabo de Guerra — {titulo}</h1>
          <p className="mt-1 text-sm text-neutral-500">
            Duas equipes disputam {totalRodadas} perguntas de múltipla escolha — quem responde certo primeiro puxa a
            corda pro seu lado.
          </p>
  ...
  return (
    <main className="flex min-h-screen flex-col bg-[#0d0d1a]">
      ...
      <div className="flex items-center justify-between border-b-4 border-[#FFD600] bg-black/30 px-4 py-2">
        ...
        <div className="flex flex-col items-center gap-0.5">
          ...
          <p className="text-xs text-neutral-400">
            Pergunta <b className="text-white">{rodada}</b>/{totalRodadas}
          </p>
        </div>
      ...
      <div className="border-b-4 border-black/25 bg-gradient-to-br from-[#FFD600] to-[#FF8F00] px-4 py-3 text-center">
        ...
        <p className="text-2xl font-extrabold tracking-wide text-[#1a1a2e]">{perguntaAtual?.enunciado ?? "..."}</p>
      </div>
      ...
      <div className="grid flex-1 grid-cols-[1fr_1.2fr_1fr]">
        <div className="flex flex-col items-center justify-center gap-3 bg-gradient-to-b from-[#0d47a1] to-[#1565C0] p-3">
          ...
          {perguntaAtual && (
            <Alternativas
              opcoes={perguntaAtual.alternativas}
              indiceCorreto={perguntaAtual.indiceCorreto}
              selecionado={selecao1}
              status={status1}
              revelar={revelar}
              onEscolher={(indice) => escolher(1, indice)}
            />
          )}
        </div>
        ...
        <div className="flex flex-col items-center justify-center gap-3 bg-gradient-to-b from-[#b71c1c] to-[#C62828] p-3">
          ...
          {perguntaAtual && (
            <Alternativas
              ...
              status={status2}
              revelar={revelar}
              onEscolher={(indice) => escolher(2, indice)}
            />
          )}
        </div>
      </div>
